...
import { ServiceId, TierId, isValidTierId } from "@/lib/types/services";
...
export function ServiceSection({ serviceId, selectedTierId, onSelect }: ServiceSectionProps) {
  ...
  return (
    <motion.div
      ...
      className={cn("rounded-xl p-8 transition-colors", selectedTierId && "bg-muted")}
    >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        {(Object.entries(service.tiers) as [TierId, typeof service.tiers[TierId]][]).map(
          ([tierId, tier]) => (
            <ServiceCard
              key={tierId}
              name={tier.name}
